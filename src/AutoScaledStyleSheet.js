...
const autoScaledSheetCreator = (scale, verticalScale, horizontalScale, moderateScale) => {
    const autoScaleFunc = autoScaleByAnnotation(scale, verticalScale, horizontalScale, moderateScale);
    return {
        create: styleSheet => StyleSheet.create(deepMapStyleSheet(styleSheet, autoScaleFunc))
    }
}
